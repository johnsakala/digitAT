...
class GroupVoiceCallsHandler {
  ...
  void bye() async {
    ...
    if (isCaller && !callStarted) {
      ...
      await FireStoreUtils.firestore
        ...
        (value.data['connections'] as Map<String, dynamic>)
          ..clear();
    ...
    if (isCaller || callStarted && callAnswered) {
      if (mapData.containsKey('hangup') &&
        ...
        if (_remoteStreams.containsKey(connectionID) &&
          ...
          _listOfStreams
              .remove(_remoteStreams[connectionID]
              .getAudioTracks()
              .first);
  ...
  Future<RTCPeerConnection> _createPeerConnection(String connectionID) async {
    ...
    pc.onAddStream = (stream) {
      ...
      _listOfStreams.add(_remoteStreams[connectionID]
          .getAudioTracks()
          .first);
      if (this.onStreamListUpdate != null) this.onStreamListUpdate(
          _remoteStreams.values.toList());
    };
  ...
  _sendOffer(String token, Map<String, dynamic> data,
    ...
    if (isCaller) {
      ...
      request['conversationModel'] = homeConversationModel.conversationModel
          .toJson();
      await FireStoreUtils.firestore
          .collection(USERS)
          .document(data['to'])
        ...
        if (value.documents.isEmpty) {
          ...
          showAlertDialog(context, 'call', 'userHasAnOnGoingCall');
        }
      });
    } else {
      FireStoreUtils.firestore.collection(USERS).document(_selfId).collection
        (CALL_DATA).document(callerID)
          .get(source: Source.server).then((value) async {
        ...
        await FireStoreUtils.firestore.collection(USERS).document(_selfId)
            .collection
          (CALL_DATA).document(callerID).setData(request, merge: true);
      });
    }
  }
      ...
      if (member.userID != _selfId) {
        ...
        mapOfUserListener[getConnectionID(member.userID)] =
            messagesStream.listen((call) {
              print('GroupVoiceCallsHandler.listenForMessages ${getConnectionID(
                  member.userID)}');
              if (call != null && call.exists) {
                onMessage(call.data, getConnectionID(member.userID));
              }
              else {
                if (!isCaller && !call.exists && !callStarted &&
                    getConnectionID(
                        member.userID).contains(callerID)) {
  ...
  _sendAnswer(Map<String, dynamic> data) async {
    ...
    var request = new Map<String, dynamic>();
    FireStoreUtils.firestore.collection(USERS).document(_selfId)
        .collection(CALL_DATA).document(callerID)
        .get(source: Source.server)
        .then((callDocument) async {
      Map<String, dynamic> connections = callDocument.data['connections'] ??
          Map<String, dynamic>();
      connections[getConnectionID(data['to'])] = data;
      request['type'] = 'answer';
      request['isGroupCall'] = true;
      ...
      await FireStoreUtils.firestore.collection(USERS).document(_selfId)
          .collection(CALL_DATA)
          .document(callerID).setData(request, merge: true);
    });
  }
      ...
      if (receiverID != callerID && _selfId.compareTo(receiverID) < 0) {
        ...
        FireStoreUtils.firestore.collection(USERS).document(_selfId).collection
          ...
          if (!connections.containsKey(getConnectionID(receiverID))) {
            RTCPeerConnection pc = _peerConnections[getConnectionID
              (receiverID)];
            RTCSessionDescription offer = await pc.createOffer(_constraints);
            pc.setLocalDescription(offer);
            await _sendOffer('', {
  ...
  _deleteMyCallFootprint() async {
    ...
    await Future.forEach(homeConversationModel.members, (member) async {
      ...
      await FireStoreUtils.firestore.collection(USERS).document(member.userID)
          .collection(CALL_DATA)
          .document(isCaller ? _selfId : callerID)
          .delete();
      print('GroupVoiceCallsHandler._deleteMyCallFootprint ${member.userID}');
   ...
   // HomeScreen.onGoingCall = false;
    callStarted = false;
    callAnswered = false;
    if (this.onStateChange != null) this.onStateChange(SignalingState
        .CallStateBye);
  }
