...
class HomeOptions extends StatefulWidget {
  final User user;
  const HomeOptions( {Key key, this.user}) : super(key: key);
...
//  showAlertDialog(context);
    final http.Response response = await http
        .get(
          '${webhook}crm.company.list?filter[ASSIGNED_BY_ID]=10',
        )
...
int contactID;
 ...
 @override
  void initState() {
    super.initState();
 _fetchDoctors().then((value) {
   setState(() {
     allmeds.addAll(value);
   });
   allmeds.addAll(value);
 });
    SharedPreferences.getInstance().then((value) {
    value.getInt('contactID');
    });
  } 
        ...
        child: Column(
          children: <Widget>[
            ...
            Container(
                  ...
                  child: Center(
                    child:Text('My Partners ', style: TextStyle(
                    fontWeight: FontWeight.bold,
                    color: Colors.black,
                    fontSize: 15,
...
//  showAlertDialog(context);
 ...
 Future<List<Doctor>> _fetchPartners() async {
//  showAlertDialog(context);
    final http.Response response = await http
        .get(
      '${webhook}crm.company.list?filter[ASSIGNED_BY_ID]=10&filter[CONTACT_ID]=$contactID',
    )     .catchError((error) => print(error));
    Map<String, dynamic> responseBody = jsonDecode(response.body);
    List serverResponse = [];
    List<Doctor> _doctorsList = [];
          ...
          Container(
            ...
            child: Row(
              ...
              children: <Widget>[
                ...
                IconButton(
                  onPressed: (){
                   confirmDialog( context, "Select ${data.elementAt(index).name} Partner to provide Healthcare services with them!");
                  },
                  icon: Icon(Icons.info),
